/**
 @author : Yu Li
 gulp-concat: 合并文件js和css
 gulp-uglify: 压缩js文件
 gulp-rename: 文件重命名
 gulp-less：编译less
 gulp-clean-css：压缩css
 gulp-livereload：实时自动编译刷新
 gulp.src(filepath): 读取文件流对象
 gulp.dest(filepath): 指向所有的文件夹用于像文件中输出问价
 gulp.watch()：监控文件的变化
 **/
...
import concat from 'gulp-concat' // 合并文件js和css文件
...
async function buildNamespace (options) {
  await esbuild.build({
    ...
    format: 'iife',
    globalName: options.node ? 'ns' : 'edd.__namespace',
...
async function buildCSS (options) {
  await esbuild.build({
    ...
    outfile: path.join('dist', 'edd.css'),
  })
}

async function buildModules (options) {
  const dcPath = path.join('src', 'edd.js')
...
async function addCopyright (options) {
  ...
  if (options.iife) {
    let filePath = path.join('dist', 'edd.js')
...
async function regenerate (option, content) {
  await fse.remove('dist/edd.js')
  await fse.remove('dist/edd.css')
